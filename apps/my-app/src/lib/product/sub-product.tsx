...
const SubProduct = ({
    ...
    CreateSubProduct({
      variables: {
        createSubProductInput: {
          ...
          attributes: categoryData?.categories?.find((val: ICategory) => val?._id === category)
            ?.attributes,
  ...
  const handleAddVariantImage = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      setSubValue((prevVariant) => ({ ...prevVariant, image: [...prevVariant.image, valueImage] }));
  ...
  return (
    <div className="py-4 px-4 bg-white min-w-full border-0 rounded-md ">
              ...
              <div key={attribute._id}>
                <div className=" font-medium">
                  <Select
                    ...
                    className="block w-full h-12 rounded-md border-0 mt-2 p-3  font-semibold bg-white text-gray-700 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 sm:text-sm sm:leading-6"
